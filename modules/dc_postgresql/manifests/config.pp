...
class dc_postgresql::config {
  ...
  if $dc_postgresql::params::cluster_master_node == $::fqdn {
    ...
    if member(query_nodes("Ssh_authorized_key[barman_key_${::dc_postgresql::params::backup_server}]"), $::fqdn) and member(query_nodes("Ssh_authorized_key[postgres_backup_key_${::hostname}]"), $dc_postgresql::params::backup_server) {
