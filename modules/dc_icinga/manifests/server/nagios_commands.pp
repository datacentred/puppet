...
class dc_icinga::server::nagios_commands {
  ...
  icinga::command { 'check_nova_instance':
    command_line => "/usr/lib/nagios/plugins/cache_check.py -c \"/usr/lib/nagios/plugins/check_nova-instance.py --auth_url https://\$HOSTALIAS\$:${keystone_post}/v2.0 --endpoint_url https://\$HOSTALIAS\$:${nova_osapi_port}/v2 --tenant ${keystone_icinga_tenant} --username ${keystone_icinga_user} --password ${keystone_icinga_password} --instance_name icinga --image_name CirrOS\ 0.3.2\ x86_64 --flavor_name m1.tiny --force_delete\" -e 900 -t 180 -i 600"
  }
